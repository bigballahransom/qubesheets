 ...
 */
export function useVideoChapters({ projectId, recording, videoId, currentTime, enabled = true, refreshKey = 0 }) {
  ...
  useEffect(() => {
    ...
    // refreshKey lets callers force a refetch when analysis lands mid-open
    // (mid-call processed recordings) without changing the recording identity.
  }, [enabled, projectId, recording?._id, videoId, refreshKey]);
