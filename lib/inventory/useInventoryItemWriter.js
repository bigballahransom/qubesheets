...
export function useInventoryItemWriter({
  ...
  useEffect(() => {
    return () => {
      ...
      pendingRef.current = {};
      if (Object.keys(changes).length === 0) {
        // Nothing pending, but the row may still be flagged dirty (e.g. a
        // flush was mid-flight when the modal closed). Release the flag so
        // the parent's refetch merge doesn't ignore this item forever.
        if (markCleanRef.current) markCleanRef.current(itemId);
        return;
      }
      ...
      // Release the dirty flag once the PATCH settles — leaving it set
      // blocked every future poll/refetch update for this item for the
      // rest of the session (stale data that looks like a revert/mismatch).
      fetch(apiBase, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(changes),
  ...
  // Synchronous read of the latest optimistic state (stateRef), for callers
  // that need to compute a change AND echo it elsewhere (e.g. optimistic
  // mergeUpdatedItem) without the stale-closure risk of render-time `state`.
