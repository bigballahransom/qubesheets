...
class SimpleRealTimeDatabase {
  ...
  startPolling(projectId, callback, intervalMs = 5000) {
  ...
  stopPolling(projectId) {
    ...
    if (intervalId) {
      ...
      // BUGFIX: was delete(intervalId) — the map is keyed by projectId, so the
      // stale entry was never removed and intervals accumulated across re-mounts.
